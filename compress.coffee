...
compress = (inputFile, outputFile) ->
  logger 'starting to compress the file ' + inputFile
...
writeBufferToFile = (outputBuffer, length, filename) ->
  sliceBuffer = outputBuffer.slice 0, length
...
writeHeader = (buffer, dictionary, filesize, callback) ->
  ...
  freqMap.writeDictionary buffer, dictionary, 5, (err, bytesWritten) ->
    logger 'Header was written to temp file ' + bytesWritten
...
encode = (dataBuffer, freqMap, outputFile, callback) ->
  ...
  writeHeader buffer, freqMap, dataBuffer.length, (err, sizeWritten) ->
    if err
      ...
      digest.md5digest dataBuffer, buffer, sizeWritten, (err) ->
        if err
          console.error err
        else
          nextOffset = sizeWritten + 16
          accumulator = 0 
          index = 0
          bitWriter = (bit) ->
            logger 'bit is ' + bit
